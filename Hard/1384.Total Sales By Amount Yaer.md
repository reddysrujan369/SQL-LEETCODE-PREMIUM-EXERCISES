...
SELECT p.product_id,
       ...
       s.average_daily_sales * DATEDIFF(LEAST(s.period_end,DATE(CONCAT(y.year,'-12-31'))), GREATEST(s.period_start,DATE(CONCAT(y.year,'-01-01')))) + 1
       
FROM Sales s CROSS JOIN (
SELECT 2018 AS year UNION SELECT '2019' AS year UNION SELECT '2020' AS year
) AS y
JOIN Products p ON s.product_id = p.product_id
WHERE y.year BETWEEN s.period_start AND s.period_end
```
