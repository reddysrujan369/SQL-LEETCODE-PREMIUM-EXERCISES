...
WITH PassesWithTeams AS (
    SELECT
        ...
        IF(t1.team_name = t2.team_name, 1, 0)  AS same_team_flag,
        p.time_stamp
    FROM Passes p
...
StreakGroups AS (
    SELECT
        team_from,
        same_team_flag,
        SUM(IF(same_team_flag = 0, 1, 0))
            OVER (PARTITION BY team_from ORDER BY time_stamp) AS group_id
...
| CTE | What it does |
|-----|-------------|
| PassesWithTeams | Join passes with teams, flag if same team |
| StreakGroups | Running sum of breaks creates group_id |
| Final SELECT | Count rows per island, pick the longest |

## Key Concepts
- Gaps and Islands
- Window Function SUM() OVER()
- MAX(COUNT(*)) to find longest group
