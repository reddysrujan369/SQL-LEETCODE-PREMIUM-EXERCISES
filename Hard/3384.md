...
spark = SparkSession.builder.appName("Dominance").getOrCreate()

cte = (
    passes
    .join(teams.alias("t1"), col("pass_from") == col("t1.player_id"))
    .join(teams.alias("t2"), col("pass_to")   == col("t2.player_id"))
    .withColumn("half_number",
        when(col("time_stamp") <= "45:00", 1).otherwise(2)
    )
    .withColumn("dom",
        when(col("t1.team_name") == col("t2.team_name"), 1).otherwise(-1)
    )
)

result = (
    cte
    .groupBy("t1.team_name", "half_number")
    .agg(spark_sum("dom").alias("dominance"))
    .orderBy("t1.team_name", "half_number")
)
