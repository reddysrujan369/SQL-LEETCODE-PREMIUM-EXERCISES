...
```sql
WITH T AS(
SELECT failed_date AS dt,
       'failed' AS st
FROM Failed
WHERE YEAR(failed_date) = 2019
UNION ALL
SELECT success_date AS dt,
       'succeeded' AS st
FROM Success
WHERE YEAR(success_date) = 2019
),
R AS(
SELECT T.*,SUBDATE(dt,ROW_NUMBER() OVER(PARTITION BY st ORDER BY dt)) as grp_key
)
SELECT st AS period_state,
