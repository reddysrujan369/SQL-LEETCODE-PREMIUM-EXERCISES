...
SELECT DATE_FORMAT(pay_date,'%Y-%m') AS pay_month,
       e.department_id,
       AVG(s.amount) OVER(PARTITION BY s.pay_date) AS avg_comp,
       AVG(s.amount) OVER(PARTITION BY s.pay_date,e.department_id) AS avg_person_sal
FROM Salary s JOIN Employee e ON s.employee_id = e.employee_id
)
SELECT DISTINCT pay_month,
       department_id,
       CASE WHEN avg_comp = avg_person_sal THEN 'same'
            WHEN avg_comp > avg_person_sal THEN 'lower'
