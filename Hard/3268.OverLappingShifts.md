...
EmployeeShifts
      │
      ▼
CTE T → Collect all unique time points (start + end times)
      │
      ▼
CTE P → Create intervals using LEAD (st → ed)
      │
      ▼
CTE S → JOIN back to EmployeeShifts to count overlapping shifts per interval
      │
      ├──────────────────────────────────┐
      ▼                                  ▼
CTE R → Self JOIN EmployeeShifts      CTE S
        to find overlapping pairs       MAX(concurrent_count)
        SUM overlap minutes
      │                                  │
      └──────────────┬───────────────────┘
                     ▼
...
| CTE     | Complexity  | Reason                          |
|---------|-------------|---------------------------------|
| T       | O(N log N)  | UNION deduplication             |
| P       | O(N log N)  | LEAD window function            |
| S       | O(N × K²)   | JOIN intervals × shifts         |
| R       | O(N × K²)   | Self JOIN shift pairs           |
| Final   | O(N log N)  | GROUP BY + ORDER BY             |
| Overall | O(N × K²)   | S and R are the expensive parts |

```
N = total rows in EmployeeShifts
K = max shifts per single employee

If K is small (5-10 shifts)  → practically O(N)
If K is large (100+ shifts)  → becomes very expensive
```
...
-- Helps CTE S and CTE R JOIN conditions
CREATE INDEX idx_employee_times
ON EmployeeShifts (employee_id, start_time, end_time);

-- Index 2: Helps CTE T collect distinct time points
CREATE INDEX idx_start_time
ON EmployeeShifts (employee_id, start_time);

CREATE INDEX idx_end_time
ON EmployeeShifts (employee_id, end_time);

-- Index 3: Helps final GROUP BY and ORDER BY
CREATE INDEX idx_employee_id
ON EmployeeShifts (employee_id);
```
...
| Index                | Helps          | Why                              |
|----------------------|----------------|----------------------------------|
| idx_employee_times   | CTE S, CTE R   | Filters on employee + time range |
| idx_start_time       | CTE T          | Avoids full scan for start times |
| idx_end_time         | CTE T          | Avoids full scan for end times   |
| idx_employee_id      | Final SELECT   | GROUP BY and ORDER BY            |
