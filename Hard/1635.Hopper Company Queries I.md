...
T AS (
SELECT MONTH(requested_at) AS month,COUNT(*) AS cnt
FROM Rides JOIN AcceptedRides using (ride_id)
WHERE YEAR(requested_at) = 2020
)
SELECT m.month,
       COUNT(d.driver_id) AS active_drivers,
       cnt AS accepted_rides
FROM months m LEFT JOIN Drivers d ON (m.month >= MONTH(join_date) AND YEAR(join_date) = 2020) OR YEAR(join_date) < 2019
              LEFT JOIN T t ON m.month = r.month
```
