...
R AS(
SELECT player,group_id,ROW_NUMBER() OVER(PARTITION BY group_id ORDER BY score desc,player ASC) AS rn
FROM P
)
SELECT group_id,
       player AS player_id      
...
Q AS(
SELECT p.player_id,p.group_id,COALESCE(SUM(t.score),0) AS score
FROM Players p
LEFT JOIN T AS t ON p.player_id = t.player_id
GROUP BY p.player_id,p.group_id
),
R AS(
SELECT player_id,group_id,ROW_NUMBER() OVER(PARTITION BY group_id ORDER BY score desc,player_id ASC) AS rn
FROM Q
)
SELECT group_id,
       player AS player_id      
