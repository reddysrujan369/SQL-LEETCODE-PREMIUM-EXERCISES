...
concurrent AS(
    SELECT m.*,LEAD(st) OVER (PARTITION BY employee_id ORDER BY st) as ed
),
streak AS(
   SELECT C.employee_id,
          COUNT(1) AS count
  FROM concurrent C INNER JOIN Tasks T ON C.employee_id = T.employee_id
  WHERE C.st >= T.start_time AND C.ed <= T.end_time
  GROUP BY 1
)
SELECT T.employee_id,
       FLOOR(SUM(TIMESTAMPDIFF(second,start_time,end_time))/3600) AS total_task_hours,
       MAX(count) as max_concurrent_tasks
FROM Tasks T LEFT JOIN streak S ON T.employee_id = S.employee_id
ORDER BY 1 ASC
```
``` sql2
WITH merg AS(
...
concurrent AS(
    SELECT m.*,SUM(delta) OVER(PARTITION BY employee_id ORDER BY st,delta desc) AS cont
FROM merg
),
streak AS(
   SELECT C.employee_id,
          MAX(cont) as max_concurrent_tasks
  FROM concurrent C 
  GROUP BY 1
)
SELECT T.employee_id,
...
FROM Tasks T INNER JOIN streak S ON T.employee_id = S.employee_id
ORDER BY 1 ASC
```
