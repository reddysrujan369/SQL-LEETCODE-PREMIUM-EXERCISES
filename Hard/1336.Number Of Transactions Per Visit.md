...
WITH RECURSIVE tcount AS (
   ...
   WHERE n < (
       SELECT MAX(tcnt)
       FROM (
           SELECT user_id, transaction_date, COUNT(*) AS tcnt
           FROM Transactions
...
S AS (
   SELECT v.user_id,
          v.visit_date,
          COALESCE(t.tcnt, 0) AS tcnt 
   FROM Visits v 
   LEFT JOIN T t ON v.user_id = t.user_id AND v.visit_date = t.transaction_date
)
SELECT tc.n AS transactions_count,
       COUNT(s.user_id) AS visits_count
FROM tcount tc 
LEFT JOIN S s ON tc.n = s.tcnt
GROUP BY 1
ORDER BY 1;
```
