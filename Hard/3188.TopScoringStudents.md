...
GROUP BY S.student_id  -- Fixed: GROUP BY student_id
HAVING 
    -- Condition 1: All mandatory courses = 'A'
    SUM(CASE WHEN C.mandatory = 'yes' AND E.grade = 'A' THEN 1 ELSE 0 END) = 
    SUM(CASE WHEN C.mandatory = 'yes' THEN 1 ELSE 0 END)
    
    -- Condition 2: All TAKEN non-mandatory courses = 'A' or 'B'
    AND SUM(CASE WHEN C.mandatory = 'no' AND E.grade IS NOT NULL THEN 1 ELSE 0 END) = 
        SUM(CASE WHEN C.mandatory = 'no' AND E.grade IS NOT NULL AND E.grade IN ('A', 'B') THEN 1 ELSE 0 END)
    
    -- Condition 3: At least 2 non-mandatory courses TAKEN
